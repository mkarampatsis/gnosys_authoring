...
function _brImportLocalFile(filename) {
  if (
    ...
    self._brFilesObj[filename].type === 'text'
  ) {
...
function _brInitMsgSenders() {
  ...
  self._brStderrWrite = function (data) {
    if (
      (data + '').startsWith('Traceback (most recent call last):') &&
      data === self._brPrevErrOut
    ) {
      ...
      value: function open() {
        ...
        if (params.length > 1) {
          ...
          if (url.startsWith(this.localPrefix)) {
            var localPath = url.slice(
              this.localPrefix.length,
              url.indexOf('?')
            );
        ...
        return (_get2 = (0, _get4['default'])(
          (0, _getPrototypeOf2['default'])(_brXHR.prototype),
          'open',
          this
        )).call.apply(_get2, [this].concat(params));
      },
    },
    {
      key: 'send',
      value: function send() {
        if (this.localRequestOpened) {
          ...
          return (_get3 = (0, _get4['default'])(
            (0, _getPrototypeOf2['default'])(_brXHR.prototype),
            'send',
            this
          )).call.apply(_get3, [this].concat(params));
        }
      },
    },
    {
